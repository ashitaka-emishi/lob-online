...
| 2     | #487  | CounterImageWidget: release activeFace on blur to bound global keydown interception | PR #485       | The global `window` keydown listener suppresses Arrow key scrolling while a slot is active; if the user navigates away without pressing Escape, scrolling remains suppressed. Dev tool with no public user impact. Fix: add `@focusout` handler releasing `activeFace` when focus leaves the widget. |
| 2     | #486  | CounterImageWidget: pass side as explicit prop from OobDetailPanel                  | PR #485       | Side detection logic is duplicated between `OobDetailPanel` and `CounterImageWidget`, coupling the widget to the `leaders.` path prefix convention. A format change requires updating both files; union/confederate cut-out filtering could silently break. Bounded by the confederate-path test.    |
