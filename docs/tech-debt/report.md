...
_Last updated: 2026-05-23 after PR #451._
...
| -------------------------------- | ----------------------------------------------------------------------------------- |
| Open debt items                  | 19                                                                                  |
| Cumulative debt score (net open) | 34                                                                                  |
| Highest-risk item                | refactor: move contour coexistence/replace logic into edge-model.js (#453, score 3) |
| PRs tracked                      | 208                                                                                 |
...
PR #451 (map editor bug fixes + team review) added 16 points across 8 new items, bringing the net open score from 18 to 34. One score-3 item was introduced: #453 (contour coexistence logic split across view and formula layer — a layering inversion that will mislead future callers of `validateCoexistence`). Current debt is concentrated in three categories: (1) the new score-3 layering issue and six score-2 items from the PR #451 review (composable API smell, missing test coverage for new behaviors, migration watcher placement, AT gaps); (2) seven pre-existing score-2 items — sequential fetch latency (#440), four rules-engine stubs (#379 #381 #382 #383 — will close at M6), and two security/infra items (#350 #403 — deferred to M8); and (3) five score-1 items with no milestone pressure. A targeted debt sprint on the PR #451 findings (#452–#459) would return the score to ~18 and eliminate the layering inversion before M6 combat complexity lands.
...
| ----- | ----- | -------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 3     | #453  | refactor: move contour coexistence/replace logic into edge-model.js              | PR #451       | `handleEdgePaint` performs contour replace semantics inline, bypassing `validateCoexistence`. Two policies now exist for the same rule — a future caller of `validateCoexistence` will be rejected on input the map editor silently accepts. Layering inversion; fix before M6 adds more edge-paint callers. |
| 2     | #459  | a11y: add aria-pressed to contour/terrain type buttons and humanize labels       | PR #451       | Type-selection buttons carry no `aria-pressed`; camelCase labels (`extremeSlope`, `verticalSlope`) read poorly to screen readers. Template-only fix.                                                                                                                                                         |
| 2     | #458  | test: add coverage for migration watcher and active-highlight prop fix           | PR #451       | Two behavioral fixes from PR #451 (migration watcher, `.value` unwrap) have no regression tests. Regressions would be silent.                                                                                                                                                                                |
| 2     | #457  | test: add coverage for handleEdgePaint contour replace semantics                 | PR #451       | Strip-then-add contour replace (the core #417 fix) has zero test coverage. Regression would revert bug #417 silently.                                                                                                                                                                                        |
| 2     | #455  | refactor: move 'unknown'→'clear' migration to load boundary in useMapPersistence | PR #451       | Migration watcher lives in the view layer, re-scans all 2261 hexes on every map replacement, and does not mark data as migrated. Belongs at the data ingress boundary.                                                                                                                                       |
| 2     | #454  | refactor: fix useEdgePanelWiring composable to not require .value in templates   | PR #451       | Composable returns a plain object containing a Ref; Vue 3 does not auto-unwrap nested refs in templates. Every consumer must remember explicit `.value` — invisible to the type system.                                                                                                                      |
| 2     | #452  | refactor: unify contour-type list to single source of truth                      | PR #451       | The same 4-value contour type set exists in three places. Adding a fifth type requires editing all three in lockstep.                                                                                                                                                                                        |
...
| 1     | #456  | test: add geometry-coordinate assertions to useEdgeLineLayer tests               | PR #451       | Composable switched from `edgeLine20_80` to `edgeLineFull` but no test verifies `lineAttrs` coordinates. A revert or broken geometry passes all current tests.                                                                                                                                               |
