...
_Last updated: 2026-06-14 after PR #585 (m6-ui_20260614 team-review)._
...
| 2     | M6        | #583  | refactor: move scenario fetch into useGameStore (scenarioId source of truth)         | PR #585       | GameView fetches scenario data inline via route params rather than via `useGameStore`. Two bundled issues: fetch placement (should live in `loadGame` alongside map-config) and dual source of truth (route slug vs `gameState.scenarioId` can diverge for redirected URLs, causing TurnControl to show wrong scenario lighting). Low urgency in current single-user deployment. |
| 2     | M6        | #584  | a11y: add `<main>` landmark to MenuLayout for screen-reader navigation               | PR #585       | `MenuLayout.vue` wraps slot content in a generic `<div>`, so `HomeView` and `AboutView` expose no `<main>` landmark for screen-reader navigation (WCAG 1.3.1). Pre-existing issue; simple one-line fix. Low impact for current dev-only deployment.                                                                                                                              |
