...
_Last updated: 2026-05-31 after PR #475._
...
PR #475 (M5 closeout: game action API, Socket.io rooms, GameView design, info panel) fixed 11 findings in place and deferred 7 items totalling 19 debt points, raising net open score from 25 to 43 (after closing #387). The new debt is concentrated in five score-3 items: an auth boundary gap (requireSide never validates stored side tokens — #477), server-fault error message leakage exposing Zod internals (#478), a brittle all-or-nothing OOB fetch that lets a transient leaders outage blank all unit enrichment (#479), missing test coverage for the new leaders fetch dependency (#476), and WCAG non-conformance in the interactive counter layer (#480). These should be prioritized in a debt sprint before M6 combat/morale work begins. The remaining score-2 items include rules-engine stubs (#379 #381 #382 #383 — safe until M6 combat phases land), test quality improvements (#481), infrastructure hardening (#482), and pre-existing map-editor and security/infra items from earlier PRs.
