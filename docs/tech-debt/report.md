...
_Last updated: 2026-06-04 after PR #493._
...
| -------------------------------- | --------------------------------------------------------------------------------------------------- |
| Open debt items                  | 18                                                                                                  |
| Cumulative debt score (net open) | 36                                                                                                  |
| Highest-risk item                | test(gameView): add localPlayerSide fetch and validActions derivation test coverage (#494, score 3) |
| PRs tracked                      | 233                                                                                                 |
...
PR #493 (M5 UI completion) introduced 7 new deferred findings (+17 points), bringing the net open score from 19 to 36. The 18 open items now fall into four clusters: (1) **client/server divergence** — #494 (untested `validActionsForState`/identity fetch, score 3) and #495 (client action derivation stub that must be replaced before M6 adds action types, score 3) are the highest-priority pre-M6 items; (2) **accessibility gaps** — #497 (no live-region for turn-handoff or pending state, score 3), #498 (no landmark/group labeling, score 2) address ActionPanel a11y and should accompany the next UI sprint; (3) **client-side cleanup** — #496 (silent identity-fetch failure, score 2), #499 (module-level socket state, score 2), #500 (spinner targeting by position, score 2) are low-risk but should be addressed before multi-action M6 work; (4) **pre-existing stubs/infrastructure** — #492 (map editor route layering, score 3), #403 (CSP, score 2), #383 #382 #381 #379 (rules-engine stubs safe until M6 combat, score 2 each), #350 (rate limiting, score 2), and four score-1 items. Items #494 and #495 are the most urgent: #494 leaves a primary new behavior untested and #495 creates a duplicate-logic trap that will silently break when M6 adds new action types.
...
| 3     | #494  | test(gameView): add localPlayerSide fetch and validActions derivation test coverage                  | PR #493       | `GET /api/v1/games/me` fetch and `validActionsForState` (GameView.vue:31–72) are entirely untested. A regression in identity-fetch or derivation goes undetected. Primary new behavior in Phase 3 with no coverage.                                                                                                                                                                                                                                                                                                    |
...
| 2     | #499  | refactor(gameView): move \_socket and \_mountedGameId from module scope to script setup scope        | PR #493       | Module-level mutable state shared across all GameView instances. `_` prefix misleadingly signals "unused" per project convention. Latent multi-instance hazard; safe today as GameView is a singleton.                                                                                                                                                                                                                                                                                                                 |
