...
_Last updated: 2026-06-05 after PR #507._
...
| Highest-risk item                | test(oob-editor): fix ineffective #506-3 guard test; guard JSON.parse scalars (#508, score 3) |
| PRs tracked                      | 238                                                                                           |
...
The OOB editor bug sprint (PR #507) fixed four display/data bugs but the team-review surfaced 5 new findings (+8 pts), bringing the net open score from 24 to 32 across 19 items. The 19 open items fall into five clusters: (1) **test coverage** — #508 (score 3, highest) groups three gaps: an inert guard test for the `#506-3` walker-phantom fix, a missing `_nodePath` forwarding test, and unguarded `JSON.parse` scalars that can corrupt `specialRules` data; #509 (score 2) adds a guard test for the `'union.hq'` literal; #504 (score 2) awaits the #492 shared-module extraction; (2) **architecture/coupling** — #502 and #503 (score 2 each) are low-risk today but widen with M6 action expansion; (3) **accessibility** — #505 (score 2, focus not restored after pending clears) and #512 (score 1, textarea labels + rows) are keyboard/SR gaps in the OOB editor and game view; (4) **M6/M8 blocked** — #383 #382 #381 #379 (rules-engine stubs awaiting M6 combat data), #403 #350 (auth hardening deferred to M8); (5) **process/trivial** — #510 #511 (score 1 each, OOB editor test polish and unchecked spec criteria) and score-1 nice-to-haves #469 #385 #205 #204. The score rise reflects review diligence; #508 is the only item with correctness implications (scalar JSON.parse) and should be closed before M6 OOB data entry begins.
...
| ----- | ----- | ------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3     | #508  | test(oob-editor): fix ineffective #506-3 guard test; add \_nodePath forwarding test; guard JSON.parse scalars | PR #507       | The `#506-3` variantIds guard has no effective test (variant in succession fixture never reaches `buildLeadersMap`); `_nodePath` forwarding from `OobTreeNode` is untested; `JSON.parse` accepts valid scalars (number/bool/null) producing type-corrupted `specialRules` data. |
| 2     | #509  | chore(oob-editor): add resolves-to-real-node test for \_nodePath 'union.hq' literal                           | PR #507       | The string `'union.hq'` in `oobTreeTransform.js:232` is disconnected from `UnionOOB.hq` in the Zod schema. A key rename silently breaks AotP HQ counter edits with no type check or test catching it.                                                                           |
...
| 1     | #510  | test(oob-editor): minor test/comment polish from PR #507 review                                               | PR #507       | Three trivial items: undocumented catch fallback, `specialRules: null` display not asserted, HQ fallback/real-entry test branches not discriminated by fixture id. No functional risk.                                                                                          |
