...
_Last updated: 2026-06-15 after PR #592._
...
| Current-milestone open debt      | 2 items (#587 score 3, M6 — ROLL_INITIATIVE side filter; #593 score 3, M6 — sync I/O on dispatch) |
...
Elevated risk. PR #592 closed 10 points (#571, #572, #573) but introduced 11 new points across 5 items, raising the net open score from 17 to 18.

Debt is now concentrated in four clusters: (1) **Auth/security scaffolding** — #562 (score 4, token/faction binding, M8 prerequisite) and #563 (score 3, re-join side-switching, M8) — intentional single-user scaffolding, hard prereqs for multiplayer; (2) **Rules-engine correctness** — #587 (score 3, M6, ROLL_INITIATIVE leader side filter — Checkpointed surface, should be addressed before M6 closes); (3) **Combat pipeline reliability** — #593 (score 3, M6, sync I/O on every action dispatch), #594 (score 3, M7, silent ctx degradation in dispatch), #595 (score 2, M7, missing route-layer integration test for FIRE_COMBAT → RESOLVE_MORALE), #596 (score 2, M7, O(n) findOobUnit walk); (4) **Production/deployment concerns and test quality** — #589 (score 3, M8, rate limiter trust proxy), #588 (score 2, M7), #383/#382/#381 (score 2 each, M7-blocked stubs), #590/#591/#597 (score 1 each). The score-4 auth item (#562) remains the highest-priority pre-M8 work. Two current-milestone items (#587, #593) should be resolved before M6 closes.
