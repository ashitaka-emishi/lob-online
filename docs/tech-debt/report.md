...
_Last updated: 2026-05-22 after PR #437._
...
PR #437 closed all 10 legacy map-config contract items (#421–#430, 17 points) and introduced 10 new minor items from the team review (#438–#447, 17 points), leaving the net open score unchanged at 46. The two score-3 items remain: #431 (OOB fetched via dev-tool endpoint) — a production-blocker where unit counters silently disappear without `MAP_EDITOR_ENABLED` — and #434 (SVG button AT reliability). Current debt is concentrated in two categories: (1) the production-blocking OOB data path (#431) that must be resolved before any production deployment of game view; and (2) sixteen score-2 items including minor architectural refinements (#438 sanitizeCalibration double-call, #440 sequential fetch latency), test brittleness (#442 503 test ordering, #446 makeMultiFetch substring matching, #447 scenarios health-check gap), and accessibility gaps (#445 banner landmark semantics, #435 overflow:hidden). Rules-engine stubs (#379, #381, #382, #383) will close naturally during M6 combat implementation. Security/infra items (#403 CSP, #350 rate limiting) remain appropriately deferred to M8 auth hardening.
...
| 2     | #447  | test: no health-check for scenarios router startup                               | PR #437       | No integration test exercises the real `loadMap` path at the server level. The 503 test covers failure; the success path (real map data, 200 + non-empty gridSpec) has no coverage.                                                                              |
| 2     | #446  | test: makeMultiFetch URL substring matching latent brittleness                   | PR #437       | Substring matching fires the first matching pattern; if a future endpoint URL is a substring of another, the wrong handler fires silently. Latent brittleness; no test broken today.                                                                             |
| 2     | #445  | a11y: banner stack uses generic divs without landmark semantics                  | PR #437       | Loading/error/warning banners are generic `<div>` elements; screen-reader users cannot navigate to the status area by landmark. Not WCAG-blocking today; worth addressing before public accessibility review.                                                    |
| 2     | #442  | test: 503 test ordering fragility in scenarios.test.js                           | PR #437       | `vi.resetModules()` in the 503 test is not wrapped in `beforeEach`/`afterEach`; test isolation depends on run order. Safe at 5-test file size; will cause flakiness as the file grows.                                                                           |
...
| 2     | #438  | arch: sanitizeCalibration called twice                                           | PR #437       | `GameView` spreads `DEFAULT_CALIBRATION` over `gridSpec` and then calls `sanitizeCalibration`; the store already calls `sanitizeCalibration` at its boundary, creating a redundant double-merge. No correctness bug.                                             |
...
| 1     | #444  | cleanup: STUB_GRID_SPEC name collision between client/server fixtures            | PR #437       | Client `STUB_GRID_SPEC` omits six calibration fields filled by `sanitizeCalibration`; the name does not signal this shape difference to contributors. Trivial; rename to `STUB_GRID_SPEC_WIRE`.                                                                  |
| 1     | #443  | cleanup: useCalibration.js re-exports create dual import paths                   | PR #437       | Both `calibration.js` and `useCalibration.js` are valid import paths for `sanitizeCalibration`; navigation confusion only, no functional risk.                                                                                                                   |
| 1     | #439  | cleanup: GridSpecSchema exported but unused in production                        | PR #437       | `GridSpecSchema` exported from `map.schema.js` has no production consumer. Trivial dead export; safe to remove or wire in when multi-scenario validation is added.                                                                                               |
