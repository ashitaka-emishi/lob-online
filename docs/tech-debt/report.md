...
_Last updated: 2026-05-21 after PR #409._
...
| -------------------------------- | -------------------------------------------------------------------------------------- |
| Open debt items                  | 19                                                                                     |
| Cumulative debt score (net open) | 35                                                                                     |
| Highest-risk item                | Implement lobby auth, ownership, and CSRF correctly (post-scaffolding) (#410, score 5) |
| PRs tracked                      | 176                                                                                    |
...
PR #409 (retroactive team-review) added 11 points across 5 new items, raising the net open score from 24 to 35. The dominant item is #410 (score 5): a bundled package of scaffolded-lobby security and correctness debt — unauthenticated DELETE IDOR, unauthorized side-switching, missing CSRF protection, inverted persistence ordering, test gaps, and missing accessibility — all intentionally deferred to M8 alongside OAuth identity work. Two minor maintainability items were also added: #411 (triplicated `session.regenerate()` pattern, score 2) and #412 (`'union'`/`'confederate'` magic strings, score 2). Two trivial items round out the addition: #413 (error color contrast) and #414 (unnecessary `/me` refetch in `deleteGame`). Seven findings from the same review were fixed in place and do not appear in the register.

Current debt is concentrated in two areas: (1) scaffolded lobby auth and correctness (#410, score 5) — the single largest item, deferred to M8 and not blocking M5/M6; (2) M6-blocked engine stubs (#379, #381, #382, #383) that are safe to leave until combat is implemented. The pre-existing score-3 item (#401, OOB composable extraction) remains the most actionable near-term item before M6 views are added. Recommended resolution: address #410 as part of the M8 auth hardening track; address #401 before any new game view is added in M6; #411 and #412 can be resolved in a focused lobby cleanup sprint.
...
| ----- | ----- | ---------------------------------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5     | #410  | Implement lobby auth, ownership, and CSRF correctly (post-scaffolding)       | PR #409       | Bundled scaffolded-lobby security and correctness debt: unauthenticated DELETE IDOR, unauthorized side-switching, no CSRF, inverted DELETE persistence ordering, missing `deleteGameFile` tests, missing deleteGame/fetchGames-failure test, no confirmation on destructive delete, missing accessible names on action buttons. All deferred to M8 alongside OAuth identity. |
...
| 2     | #411  | Extract `session.regenerate()` into a helper in `games.js`                   | PR #409       | Promise wrapper copy-pasted in three route handlers; a fix to error handling in one site will not propagate to the others. Low coupling risk today but grows with each new auth route.                                                                                                                                                                                       |
| 2     | #412  | Define SIDES constant — eliminate `'union'`/`'confederate'` magic strings    | PR #409       | Side values duplicated across server routes, client store, and tests. Rename risk is low but grows with each new endpoint or side-aware test.                                                                                                                                                                                                                                |
...
| 1     | #413  | Fix `.error` text color contrast to meet WCAG AA in LobbyView                | PR #409       | `color: red` (#FF0000) on white yields ~4.0:1 contrast, below WCAG 2.1 AA threshold of 4.5:1 for normal text. Trivial CSS fix — darken to `#c0392b` and add a non-color cue.                                                                                                                                                                                                 |
| 1     | #414  | Remove unnecessary `/me` refetch inside `deleteGame` in lobby store          | PR #409       | `deleteGame` calls `fetchGames()` on success, firing two round-trips. `/me` cannot change from deleting a game. Minor inefficiency; split into a narrower list-only refresh.                                                                                                                                                                                                 |
