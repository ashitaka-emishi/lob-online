...
| -------------------------------- | ------------------------------------------------- |
| Open debt items                  | 29                                                |
| Cumulative debt score (net open) | 51                                                |
| Highest-risk item                | OOB fetched via dev-tool endpoint (#431, score 3) |
| PRs tracked                      | 178                                               |
...
PR #420 (M5 game view viewport / map-config integration) added 22 points across 12 deferred items after fixing 11 findings in place. The two score-3 items now dominate: #431 (OOB via dev-tool endpoint) is a production-blocker — game view unit counters silently disappear in any build without `MAP_EDITOR_ENABLED`; and #401 (OOB composable extraction) will compound with every new game view added in M6–M7.

Current debt is concentrated in three categories: (1) two production-blocking OOB data path items (#431, #401) that must be resolved before M6 ships any new game view; (2) eight minor API/store contract items from PR #420 (#421–#427, #432) representing missing schema validation and route-design workarounds with low immediate risk but growing coupling cost; and (3) nine score-1 items (test coverage gaps, fixture inconsistencies, a caching omission, and pre-existing engine stubs) that are safe to address opportunistically. Recommended resolution order: #431 + #401 before any M6 game view work; the PR #420 contract items as part of a post-M5 cleanup sprint alongside #411/#412; engine stubs (#379, #381, #382, #383) close naturally during M6 combat implementation.
