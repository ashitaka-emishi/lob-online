...
| SM_MAP_DATA | `data/modules/south-mountain/map.json`     | 🔧     | 841 recorded hexes all typed, 0 `unknown` (M9, #669) — but real coverage is only ~37% of the 2240-cell grid; 6/10 VP hexes are unreachable by the movement engine. A recoverable 2261-hex dataset exists in a git stash; integration tracked in #689. Hexside features also pending; tracked in #685. |
