...
freely composable — any agent or skill may call any other skill. The three-layer ecosystem
is: **agents** (scope) → **skills** (procedure) → **orchestration** (sequencing with gate
checkpoints).
...
**Manual equivalent:** run each sub-skill's manual steps in sequence.

---

### `/issue-close`

**Purpose:** Close a GitHub issue with a merge summary comment after the PR has been
squash-merged. Resolves the merged commit SHA automatically.
...
```bash
SHA=$(gh pr view --json mergeCommit --jq '.mergeCommit.oid' | cut -c1-7)
...
**Purpose:** Regenerate all files in `docs/claude-ecosystem/` from source-of-truth inputs.
Six reference files are rebuilt fresh; four narrative files receive targeted factual
corrections. Diffs proposed content before writing.
...
```bash
# Read docs/agents/*/design.md, .claude/agents/registry.json,
# .claude/commands/*.md, docs/workflows/*/
# Rewrite each file in docs/claude-ecosystem/ to reflect current state
# Run npx prettier --write docs/claude-ecosystem/
```
