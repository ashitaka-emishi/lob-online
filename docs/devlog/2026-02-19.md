...
**`map.json`** starts as a skeleton. The `gridSpec` calibration object is present (the anchor point and hex geometry to position the overlay over `sm-map.jpg`), the VP hexes with their union/confederate point values are populated from `sm-rules.pdf`, and the entry hex lists are set. The `hexes` array has partial data — the terrain digitization work begins properly tomorrow once the calibration tool is running.
