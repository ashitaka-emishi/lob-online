...
- [ ] Task 1.1: Read `HexMapOverlay` props, `overlayConfig` shape, and `MapEditorView`
      binding site. List every flat prop that has a counterpart in `overlayConfig`.
- [ ] Task 1.2: Read all tool panel components (terrain, elevation, road, stream/wall,
      contour, wedge). Note what overlay state each logically owns.
- [ ] Task 1.3: Decide ownership mechanism — each tool panel emits its config slice upward
...
- [ ] Task 1.4: Draft the final `overlayConfig` type shape (JSDoc comment in a new
      `overlayConfig.js` constants file, or inline in `HexMapOverlay` props definition).
- [ ] Task 1.5: Write a Vitest test suite skeleton for the new `HexMapOverlay` prop API
...
_**Design decision (fill in during Task 1.3):**_

_Chosen mechanism: [emits / Pinia store] — rationale: ..._
