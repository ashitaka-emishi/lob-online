...
- [x] Task 1.1 (#508a): Fix the inert `#506-3` guard test in `oobTreeTransform.test.js`. Add a
...
- [x] Task 1.2 (#508b): Add `_nodePath` forwarding test to `OobTreeNode.test.js`. Mount
...
- [x] Task 1.3 (#508c): Guard `JSON.parse` scalars in `OobDetailPanel.vue`
...
- [x] Task 1.4 (#509): Add resolves-to-real-node test for `_nodePath 'union.hq'` literal.
      ...
      actual data key.
- [x] Task 1.5 (#510): Three trivial polish items. Add `/* invalid JSON — store raw text as
...
- [x] `npm run test` passes with all new assertions green
- [x] Mutation check: removing the `!variantIds.has(l.id)` guard causes Task 1.1 test to fail
...
- [x] Task 2.1 (#503): In `server/src/routes/games.js` (or wherever `GET /:id/actions` is
...
- [x] Task 2.2 (#504): Add a cross-implementation parity test. In
...
- [x] Task 2.3 (#505): In `ActionPanel.vue`, replace `:disabled="pending"` on action buttons
      ...
      `disabled` attribute.
- [x] Task 2.4 (#502): Move `refreshValidActions` from `GameView.vue` into `useGameStore.js`.
...
- [x] Task 3.1 (#511): Check off the five acceptance criteria in
...
- [x] Task 3.2 (#512): In `OobDetailPanel.vue` specialRules textarea, add `rows="6"`. For the
...
- [x] Task 3.3 (#469): `migrateUnknownTerrain` cannot be removed until map digitization is
...
- [x] Task 3.4 (#385): Close #385 with comment: "Property-based fuzz tests for dispatch are a
      nice-to-have. Deferring indefinitely — coverage is adequate for M5 surfaces; revisit if
      dispatch complexity grows in M7+."
- [x] Task 3.5 (#205): Close #205 with comment: "Static JSON imports are acceptable at current
      bundle size. Revisit at M8 if bundle profiling shows meaningful impact."
- [x] Task 3.6 (#204): Close #204 with comment: "200–300 watchers is acceptable at South
...
- [x] All 6 issues closed on GitHub
- [x] spec.md acceptance criteria checked off
