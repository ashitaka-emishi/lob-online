...
**Current state:** M7 complete. Rules engine foundation, all LOB v2.0 game tables, and four dev tools (Map Test, Table Test, Map Editor, OOB Editor) delivered in M1–M3. M4 added: `GameStateSchema` + `UnitStateSchema` Zod schemas, `initGameState()` engine, `gameFile`/`gameSqlite` persistence stores, games API routes with express-session, and `LobbyView` + `useLobbyStore`. M5 delivered: `UnitOrderState` schema with cross-field refinements, turn reducer + valid-actions engine (`engine/phase.js`), `POST /api/v1/games/:id/actions` endpoint with Socket.io room events, `GameView` with `UnitCounterLayer` + `UnitStatsPanel`, and `useGameStore`. M5.5 delivered: app `HomeView` with module/game selector, shared `EditorNav`, scenario-editor enhancements (lighting schedule, fog/rain, derived turn count), and the multi-module platform (#529) — `resolveModulePath` utility, 9-slug module layout under `data/modules`, `/api/v1/modules/:slug/*` routes, Vue Router module-prefixed paths, `useModuleStore`, module-level map/OOB data, and scenario start states under `scenarios/full-battle/`. M6 delivered: full combat resolution pipeline — fire combat, close combat, morale checks, morale cascade, orders resolution, rally phase, CBF removal, and Zero Rule. M7 delivered: retreat resolution (`engine/retreat.js`), interactive rally rolls, artillery action pipeline (LIMBER/UNLIMBER/FIRE_ARTILLERY/REPLENISH_ARTILLERY), end-of-turn accounting (Attack Recovery, Random Events, Variable Reinforcements), VP tracking (`engine/vp.js` with `computeVP`, `computeTerrainVP`, `computeWreckVP`, `evaluateVictory`), and game-over detection. `VpPanel.vue` and game-over overlay added to `GameView`. Starting M8 — production persistence + multiplayer. See `docs/designs/high-level-design.md` §2 for the M8 milestone plan.
...
**Map test tool** (`/tools/map-test`) — read-only rules validation tool for map-based rules. Five panels: Movement Path (lowest-cost path + per-hex cost breakdown between two clicked hexes), Movement Range (shade all reachable hexes by MP bucket for a selected unit), Hex Inspector (raw terrain/elevation/hexside data the engine sees), LOS (can-see result + plain-language blocking explanation), and Command Range (shade hexes by zone for a commander level). Backed by `engine/movement.js`, `engine/los.js`, and `engine/command-range.js`. Launch with `npm run dev:map-test`.
