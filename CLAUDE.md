...
**Current state:** M5.5 complete. Rules engine foundation, all LOB v2.0 game tables, and four dev tools (Map Test, Table Test, Map Editor, OOB Editor) delivered in M1–M3. M4 added: `GameStateSchema` + `UnitStateSchema` Zod schemas, `initGameState()` engine, `gameFile`/`gameSqlite` persistence stores, games API routes with express-session, and `LobbyView` + `useLobbyStore`. M5 delivered: `UnitOrderState` schema with cross-field refinements, turn reducer + valid-actions engine (`engine/phase.js`), `POST /api/v1/games/:id/actions` endpoint with Socket.io room events, `GameView` with `UnitCounterLayer` + `UnitStatsPanel`, and `useGameStore`. M5.5 delivered: app `HomeView` with scenario selector, shared `EditorNav`, scenario-editor enhancements (lighting schedule, fog/rain, derived turn count), and the multi-scenario platform (#529) — `resolveScenarioPath` utility, 9-slug data layout, `/api/v1/scenarios/:slug/*` routes, Vue Router scenario-prefixed paths, `useScenarioStore`, and editor composables wired to the active slug. Starting M6 — combat, morale, and orders resolution. See `docs/designs/high-level-design.md` §2 for the M6–M8 milestone plan.
